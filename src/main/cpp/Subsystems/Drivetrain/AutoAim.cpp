...
void AimFunctions::determineValues() { // ran to see what the values currently are
	...
	if ((cTarget>=6 && cTarget<=11 )|| (cTarget>=17 && cTarget<=22)){
		...
		if(distance>=distanceTarget+errorVal){
			forwardSpeed = 3; // sets speed to go forwards in fps
        }else{
			forwardSpeed = 0;
		}
